...
# THE PURE MATHEMATICAL MAX-EV CALLING RANGE VS 84.3% PUSH
OPTIMAL_CALL_STR = "AA,KK,QQ,JJ,TT,99,AKs,AKo,AJs,88,AQs,ATs,AQo,ATo,AJo,77,A9s,KQs,A7s,KTs,KJs,KQo,66,A8s,KJo,KTo,A9o,K9s,A8o,A5s,QJs,QTs,A6s,A4s,55,44,A7o,A5o,QTo,A4o,A3s,K8s,A6o,JTs,A3o,Q9s,A2s,K7s,K9o,K5s,K6s,A2o,J9s,Q9o,K8o,Q8s,Q7s,JTo,K3s,K2s,K5o,J8s,T9s,33,K4s,Q7o,K7o,Q6s,Q4s,J7s,K6o,22,Q8o,J9o,Q5s,K4o,J8o,T8s,T9o,Q6o,Q4o,K2o,Q3s,Q5o,Q2s,98s,J6s,T7s,K3o,J7o,J5s,T8o,T6s,97s,J4s,Q3o,98o,T5s,J3s,Q2o,87s,J5o,95s"
OPTIMAL_CALL_RANGE = set(OPTIMAL_CALL_STR.split(','))
...
def does_hu_push_call(hand_169, pid, pos, prior, gto, exploit_mode):
    ...
    if pid == "HERO":
        ...
        elif exploit_mode == "EXPLOIT_MATHEMATICAL":
            # GTO push from SB (Do NOT push 100%, it bleeds equity against 45% call)
            if pos == "SB": 
                freq = gto.get_push_freq(hand_169, 2, pos, prior)
                if freq < 0: freq = 0.0
                return random.random() < freq
            # Call mathematically perfect 57.16% range from BB vs 84.3% Push
            if pos == "BB": return hand_169 in OPTIMAL_CALL_RANGE
...
def main():
    ...
    evaluator = Evaluator()
    hands_dealt = 2000000  # 2 Million hands per scenario for extreme accuracy
    
    print("Running 2M Hands: Pure GTO Baseline...")
    chips_gto = run_hu_simulation(gto, evaluator, hands_dealt, "GTO", "GTO Baseline")
    
    print("Running 2M Hands: Pure Mathematical Rakeback Optimised Exploit...")
    chips_math = run_hu_simulation(gto, evaluator, hands_dealt, "EXPLOIT_MATHEMATICAL", "Math Exploit")
    
    with open("d:/aof_bot/hu_kj_math_sim.md", "w", encoding="utf-8") as f:
        f.write("# 究極の「レーキバック最適化エクスプロイト」 実戦シミュレーション\n\n")
        f.write("キングジャックとの「Heads-Up（1対1）」において、先日算出した**『レーキバックを加味して数学的に必ずプラスになる最強の57.16%コールレンジ』**を実際に装備させ、200万ハンドのシミュレーションでその威力を測定しました。\n\n")
        f.write("※ レーキ負担: 約 `3.3 bb/100`、Rakeback 70%加味。\n\n")
        
        for mode, name, chips in [
            ("GTO", "【ベースライン】Heroが純粋なGTO（無設定）でプレイした場合", chips_gto),
            ("EXPLOIT_MATHEMATICAL", "【最強エクスプロイト】Heroが数学的に導き出された『究極の57.16%コールレンジ』（SBはGTO）を使用した場合", chips_math)
        ]:
            f.write(f"## {name}\n")
