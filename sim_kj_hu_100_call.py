...
def run_hu_simulation(gto, evaluator, hands_dealt, exploit_mode):
    ...
    for i in range(hands_dealt):
        ...
        if bb_player == "13082001":
            ...
            elif exploit_mode == "EXPLOIT_CALL_100":
                bb_call = True
...
def main():
    ...
    evaluator = Evaluator()
    hands_dealt = 500000  # 50万ハンド
    
    print("Running 500k Hands: Pure GTO Baseline...")
    chips_gto = run_hu_simulation(gto, evaluator, hands_dealt, "GTO")
    
    print("Running 500k Hands: Exploit (100% Call from BB)...")
    chips_exploit = run_hu_simulation(gto, evaluator, hands_dealt, "EXPLOIT_CALL_100")
    
    with open("d:/aof_bot/hu_kj_100call_sim.md", "w", encoding="utf-8") as f:
        f.write("# 対キングジャック：BBからの「100%完全コール」戦術シミュレーション\n\n")
        f.write(f"SBから84.3%でPushしてくるキングジャックの暴走に対して、「GTO（約53%でコール）」と「絶対に降りない（100% Any Two カードでコール）」で50万ハンド対戦させた結果の比較です。\n\n")
        f.write("※ 各パターン50万ハンド実施。レーキ負担加味、Rakeback 70%加味。\n\n")
        
        for mode, name, chips in [
            ("GTO", "【ベースライン】Heroが純粋なGTO（無設定）でプレイした場合", chips_gto),
            ("EXPLOIT", "【検証】HeroがBBの時に『100%どんなゴミ手でもコール』した場合", chips_exploit)
        ]:
            f.write(f"## {name}\n")
