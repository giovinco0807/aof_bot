...
**これは OFC の学習ツール。PPPoker のプレイマネー卓が対象で、賭博ではない。**
...
**OFC を上達するための学習ツール。ギャンブルではない。**

PPPoker のプレイマネー卓が対象で、現金は一切動かない。
賭けているものが無いので、勝ち負けは目的ではなく、
**自分の判断がどれだけ最適解から外れたか**を測ることが目的。

そのためにこのボットは、勝敗ではなく**判断そのもの**を記録する:

- 直面した局面ごとに、**候補全部のランキング**を残す
- 自分が実際に置いた手が**何位だったか**、ベストとの**EV差はいくつか**を採点する
- ソルバーが答えられなかった局面も、理由を添えて残す
- **卓の人数に関わらず記録する**（今日解けない卓こそデータを取る価値がある）

`python -m ofc.recorder --mistakes 20` で、損の大きかった判断から順に並べて振り返る。
これが本体で、自動配置はおまけ。
...
エンジン側のピン表（`trainer/engine_eval.py` の `WEIGHT_FILES`）は
**モデルが昇格するたびに動く**。つまり「m3」は時期によって別物になる。

```powershell
python -m ofc.main --show-pins
```

読み込んだ重み14個の sha から指紋を作り（例 `m3:c86e3e7fac91`）、
**判断1件ごとに `decisions.engine` に記録している**。

```sql
SELECT engine, COUNT(*), AVG(ev_loss) FROM decisions GROUP BY engine;
```

**指紋の違う行同士の EV ロスは比較できない。** 採点した相手が別物だから。
学習ツールとしてはここが生命線で、これが無いと
「上達したのか、相手が変わったのか」が後から永久に分からなくなる。

エンジンの checkout が遅れていると**失敗せずに古いモデルで打つ**ので、
`python -m ofc.install` は再実行のたびに遅れを検出して知らせる（自動更新はしない）。

まだリポジトリに入っていないモデルを試すときは、重みを fixtures に置いて
`ofc/data/m3engine.json` で指名する（エンジン側リポジトリは編集しない）:
...
**不可能ではないが、Android とは難易度が別物。** 理由は一つに尽きる。

| | Android | iOS |
|---|---|---|
| フックの前提 | root + frida-server | **jailbreak** + frida-server |
| 非 root/非 JB の逃げ道 | frida-gadget を APK に埋めて再署名 | 同左。ただし **App Store の IPA は FairPlay で暗号化**されていて、patch するには復号済み IPA が要り、それを取るには普通 JB 端末が要る（鶏と卵） |
| **エミュレータ** | **root 済みのものが多数ある** | **存在しない。** Xcode の Simulator は simulator ターゲットのビルドしか動かず、App Store の ARM IPA は動かない |
| IL2CPP の形 | `libil2cpp.so`（分離モジュール） | AOT でアプリ本体バイナリに埋まる。RVA は復号済みバイナリから取り直し |

**決定的なのはエミュレータの行。** Android が現実的なのは「最初から root が通る
エミュレータ」という逃げ道があるからで、iOS にはそれが無い。
jailbreak の可否は iOS のバージョンと端末次第で、新しい組み合わせほど公開 JB が
無い。ここは実装でどうにかなる部分ではない。

**JB 無しの iPhone で残る道は画面読み取り（OCR）だけ。** フックとは別物の
劣化した経路で、フックが完全なカードデータをくれるのに対し OCR は誤読する。

ただし**差し替えは効く構造になっている。** 境界は `SolveRequest` で、
GUI の manual モードが既にパケット無しで solve に入っている（`gui.py:621`）。
OCR リーダーは新しい入力元として足せばよく、ボット全体の作り直しにはならない。
とはいえ毎ストリート「自分の手札 + 自分の盤面 + 相手の盤面」を読む必要があり、
相応の作業にはなる。

**結論: 端末を選べるなら Android。** iPhone しか無く JB もできないなら、
このリポジトリの構成（パケット駆動）は使えず、別の入力層を書くことになる。

#### どの画面に出るか

これが構成を決める。**スマホの画面に重ねて出す機能は無い。**

| 構成 | PPPoker | 推奨表示 | 自動配置 |
|---|---|---|---|
| **A. エミュレータ（PC）** | PC 上のウィンドウ | 同じ画面に並べる | **可能性あり**（`--window LDPlayer`） |
| **B. scrcpy でミラー** | 実機、PC に転送表示 | 同じ画面に並べる | 可能性あり（scrcpy ウィンドウ相手） |
| **C. 実機 + USB / LAN** | スマホの画面 | **別マシンの GUI** | 不可 |
| **D. スマホに重ねて表示** | スマホの画面 | **スマホの画面** | 要 `adb shell input swipe` |

**A が一番現実的。** エミュレータのウィンドウは普通の Windows ウィンドウなので、
`placer.py` のウィンドウ相対座標がそのまま効く。`--window` で対象名を指定できる
（既定は `PPPoker`。エミュレータ経由なら `LDPlayer` など）。

**C はスマホを見ながら PC の画面をちらちら見ることになる。** 動くが使い勝手は悪い。

**D は未実装。** ただし調べた限り、実装可能ではある。2通りある。

**D1（スマホに表示、計算は別マシン）** — Android のオーバーレイ
（`SYSTEM_ALERT_WINDOW`）を使う Kotlin アプリを新規に書き、
ソケットで助言を受け取る。`ofc/` 側は既存の `on_advice` に送信口を足すだけ。
標準 API のみで、実現性は高い。

**ケーブルは要らない。** frida-server を TCP で待たせれば、
計算する側はネットワークのどこにいてもよい（押し入れの箱でも、家のサーバでも）:
...
# 計算する側（画面を見る必要はない）
python -m ofc.main --device 192.168.1.50 --process com.pppoker.android --hero-uid <UID>
```

`--device` はアドレス（`host` / `host:port`）を受ける。実装済み・テスト済み。
ポート既定は 27042。バラ単語は device id として扱うので、
打ち間違いが知らないホストへの接続にならない。

**遅延は問題にならない。** LAN なら往復は数ミリ秒で、solve は最悪でも
初手 123 ms。卓の持ち時間（秒単位）に対して桁が2つ違う。
インターネット越し（往復 100 ms）でも余裕がある。

#### 家の外から使う

**ルータのポート開放はしないこと。** 認証の無い frida-server をインターネットに
晒すことになり、見つけた誰でもスマホ上の任意のプロセスでコードを実行できる。

正しいやり方は **Tailscale（または WireGuard）** をスマホと計算機の両方に入れること。
これで2つの問題が同時に片付く:

- **到達性。** frida は「計算機 → スマホ」の向きに繋ぐので、
  スマホ側が接続を**受けられる**必要がある。ところがモバイル回線は CGNAT の内側で、
  外から入ってこられない。メッシュ VPN はここを NAT 越えで解決する
  （ポート開放も固定 IP も不要）
- **認証。** frida-server 自体に認証は無いが、VPN の内側にしかいなければ
  自分の端末以外から触れない

```
# 両方に Tailscale を入れ、同じアカウントでログインするだけ
python -m ofc.main --device 100.64.0.2 --process com.pppoker.android --hero-uid <UID>
```

`--device` はホスト名も受けるので `--device myphone.tailnet.ts.net` でもよい。

**遅延はモバイルでも問題ない。** 往復 30〜100 ms に solve 最悪 123 ms を足しても、
卓の持ち時間（秒単位）には遠く届かない。

**切断は起きる前提で作ってある。** トンネル・回線切替・スリープでリンクは落ちる。
落ちてもキャプチャは死なず、待って張り直す。
**死んだ接続を使い回さず、device を作り直す**（使い回すと一度切れたきり
戻らなくなる）。テスト済み。

ただしハンドの途中で切れた場合、復帰後にその局面を復元できるかは
`PineRoomStatusBRC` が届くか次第。届かなければそのハンドは黙る（§1 の制限）。
...
エミュレータや scrcpy 相手での実測はしていない（実卓での検証は元々ゼロ）。
**「表示」だけなら既定動作**なので、フックさえ通れば追加実装は要らない。
