...
for h in RAW_HAND_RANKS:
    freq = gto.get_push_freq(h, 2, "BB", "A")
    # THE BUG: I didn't check if freq exists! If it's missing, get_push_freq returns -1 !
    # Wait, earlier I did: `if freq < 0: freq = 0.0`. Did I?
    if freq < 0: continue
    if freq > 0.0:
        pure_call_hands.append(h)
        pure_combos += get_combos(h)

with open("d:/aof_bot/true_pure_call.txt", "w") as f:
    f.write(f"Combos: {pure_combos}\n")
    f.write(f"Percent: {(pure_combos/1326)*100:.2f}%\n")
    f.write(",".join(pure_call_hands))
