"""Calculate EV for last week (Mar 24-30) sessions, for 2 hero IDs."""
import sqlite3, datetime, sys, io
from treys import Card, Evaluator, Deck
from pathlib import Path

OUT_FILE = Path("d:/aof_bot/ev_lastweek.txt")
...
DATE_END = "2026-03-31"  # exclusive
SESSION_GAP = 1800
EQUITY_SIMS = 1500  # slightly fewer for speed with large dataset
...
def analyze_hero(conn, hero_id):
    ...
    grand_pl = 0; grand_ev = 0.0; grand_hands = 0; grand_sd = 0; grand_mw = 0
    ...
    for si, session in enumerate(sessions):
        ...
        for hi, hand in enumerate(session):
            hp = hand["profit"]
            total_profit += hp
        ...
        luck = "LUCKY" if diff_bb < -1 else ("UNLUCKY" if diff_bb > 1 else "~")
        ...
        grand_pl += total_profit; grand_ev += total_ev; grand_hands += nh; grand_sd += sd_count; grand_mw += mw_count
    ...
    if grand_hands > 0:
        ...
        ds = "+" if gdiff >= 0 else ""
        log(f"  {'-'*70}")
        log(f"  TOTAL: {grand_hands} hands | {grand_sd} showdowns (MW:{grand_mw})")
        ...
        log(f"    Diff: {ds}{gdiff:>7.1f}BB")

def main():
    conn = sqlite3.connect(str(DB_PATH))
    log(f"{'='*75}")
    log(f"  EV ANALYSIS - Last Week ({DATE_START} ~ {DATE_END})")
    log(f"{'='*75}")
    for hid in HERO_IDS:
